...
\begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Images/Hiện thực/user_app/home/fig_home_1.png}
...
\begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Images/Hiện thực/user_app/home/fig_home_2.png}
...
\begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Images/Hiện thực/user_app/home/fig_home_3.png}
...
\begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Images/Hiện thực/user_app/home/fig_home_4.png}
...
\begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=0.35\textheight,keepaspectratio]{Images/Hiện thực/user_app/home/fig_home_5.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Images/Hiện thực/partner/fig_partner_01_revision_business_entity.png}
    \vspace{0.5em}
...
\begin{itemize}
    \item \textbf{Kết quả nộp hồ sơ:} Trang ``Registration Successful!'' xác nhận hồ sơ đã tiếp nhận thành công và đang chờ admin xét duyệt trong vòng 1--2 ngày làm việc.
    \item \textbf{Thông báo email:} Hệ thống tự động gửi email xác nhận đến địa chỉ đã đăng ký, nút ``Back to Login'' đưa đối tác quay lại màn hình đăng nhập.
\end{itemize}
...
\subsubsection{Quy trình Chỉnh sửa Hồ sơ (Application Revision)}

Khi admin đánh giá hồ sơ có lỗi và chuyển trạng thái thành \texttt{REQUIRED\_RESUBMIT}, hệ thống gửi thông báo và đối tác được chuyển đến trang Application Revision. Trang này liệt kê rõ từng mục cần cập nhật kèm Revision Note cụ thể từ admin.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_02_revision_legal_rep.png}
    \vspace{1em}
    \caption{Application Revision -- phần Business Entity với trường Phone Number cần sửa}
    \label{fig:partner_revision_business}
\end{figure}

Hình~\ref{fig:partner_revision_business} hiển thị trang Application Revision phần Business Entity:
\begin{itemize}
    \item \textbf{Trường hợp lệ:} Brand Name (Tam Binh Spa) và Tax Code đã đúng, hiển thị biểu tượng \checkmark{} màu xanh.
    \item \textbf{Trường cần sửa:} Phone Number bị đánh dấu viền đỏ kèm badge ``UPDATE'' và thông báo lỗi ``Wrong number'' từ admin.
    \item \textbf{Mục đã duyệt:} Location Details (mục 2) có tick xanh và được thu gọn, không cần chỉnh sửa.
    \item \textbf{Thanh trạng thái:} STATUS ``Awaiting Revision'' (màu cam) và nút ``Resubmit Application'' cố định ở cuối trang.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_03_revision_summary.png}
    \vspace{1em}
    \caption{Application Revision -- phần Legal Representative với trường Full Name cần sửa}
    \label{fig:partner_revision_legal}
\end{figure}

Hình~\ref{fig:partner_revision_legal} hiển thị phần Legal Representative trong cùng trang:
\begin{itemize}
    \item \textbf{Full Name:} Bị đánh dấu đỏ với Revision Note ``wrong name based on id'' -- admin xác nhận tên khai báo không khớp với CCCD.
    \item \textbf{Các trường hợp lệ:} Position, ID Type, ID Number, Date of Issue đều có tick xanh.
    \item \textbf{Identity Verification:} ID Card Front và ID Card Back hiển thị badge ``Verified'' -- đã xác thực qua OCR.
    \item \textbf{Mục Documents (4):} Đã được chấp thuận, thu gọn với tick xanh.
\end{itemize}

\subsubsection{Hoàn thiện Hồ sơ Công khai (Post-Verification)}

Sau khi được Admin phê duyệt (\texttt{APPROVED}), đối tác được chuyển đến trang ``Complete Clinic Profile'' để hoàn thiện thông tin công khai. Giao diện gồm 4 mục bắt buộc và 1 mục tùy chọn, thanh trạng thái cố định ở dưới theo dõi tiến độ (Required completion: X/4).

\paragraph{Trạng thái ban đầu -- Chưa điền thông tin}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_04_clinic_profile_branding.png}
    \vspace{1em}
    \caption{Complete Clinic Profile -- phần Clinic Branding trống (Cover Image và Logo chưa tải)}
    \label{fig:partner_clinic_branding_empty}
\end{figure}

Hình~\ref{fig:partner_clinic_branding_empty} hiển thị mục Clinic Branding khi chưa có ảnh:
\begin{itemize}
    \item \textbf{Cover image (16:9):} Vùng tải ảnh bìa, hiển thị ở đầu trang phòng khám trên ứng dụng di động.
    \item \textbf{Logo image (vuông):} Vùng tải logo, hiển thị trong các thẻ tìm kiếm, chat, và danh sách booking.
    \item \textbf{Cảnh báo:} ``Complete the required fields to unlock the dashboard. Required completion: 0/4'' hiển thị cố định dưới cùng.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_05_clinic_profile_about.png}
    \vspace{1em}
    \caption{Complete Clinic Profile -- phần About Clinic và Clinic Gallery chưa điền}
    \label{fig:partner_clinic_about_empty}
\end{figure}

Hình~\ref{fig:partner_clinic_about_empty} hiển thị các mục bắt buộc khác ở trạng thái rỗng:
\begin{itemize}
    \item \textbf{About clinic:} ``No clinic description added yet.'' -- Yêu cầu tối thiểu 120 ký tự mô tả chuyên môn, kinh nghiệm và phương pháp.
    \item \textbf{Clinic gallery:} ``No gallery images uploaded yet.'' -- Yêu cầu 3--8 ảnh (exterior, treatment rooms, equipment).
    \item \textbf{Trust badges and certifications:} Mục tùy chọn (``optional in v1''), cho phép thêm chứng nhận, tiêu chuẩn chất lượng.
\end{itemize}

\paragraph{Hoàn thiện Thông tin Phòng khám}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_07_clinic_profile_filled.png}
    \vspace{1em}
    \caption{Complete Clinic Profile -- phần Clinic Branding đã tải ảnh bìa và logo}
    \label{fig:partner_clinic_branding_filled}
\end{figure}

Hình~\ref{fig:partner_clinic_branding_filled} hiển thị Clinic Branding sau khi hoàn thiện:
\begin{itemize}
    \item \textbf{Cover image:} Ảnh bìa cơ sở (ảnh thực spa/clinic, tỷ lệ 16:9) đã được tải lên, có nút Change và Delete.
    \item \textbf{Logo image:} Logo vuông của phòng khám đã được tải lên.
    \item \textbf{Trạng thái:} ``All required sections are ready. Required completion: 4/4'' -- nút ``Complete profile'' được kích hoạt.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_08_clinic_gallery_filled.png}
    \vspace{1em}
    \caption{Complete Clinic Profile -- About Clinic đã điền mô tả và Gallery với 3 ảnh}
    \label{fig:partner_clinic_gallery_filled}
\end{figure}

Hình~\ref{fig:partner_clinic_gallery_filled} hiển thị About Clinic và Clinic Gallery đã hoàn thiện:
\begin{itemize}
    \item \textbf{About clinic:} Mô tả tổng quan đã được nhập, xuất hiện nút ``Edit description'' để chỉnh sửa.
    \item \textbf{Clinic gallery:} 3 ảnh (Image 1/2/3 of 3) đã tải lên -- phòng trị liệu, phòng thư giãn nhóm, và sảnh đón tiếp. Mỗi ảnh có nút mũi tên trái/phải (điều chỉnh thứ tự) và nút xóa.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_09_clinic_certification_modal.png}
    \vspace{1em}
    \caption{Modal ``Add certification'' -- thêm chứng nhận cho Trust Badges}
    \label{fig:partner_clinic_add_badge}
\end{figure}

Hình~\ref{fig:partner_clinic_add_badge} hiển thị modal thêm chứng nhận:
\begin{itemize}
    \item \textbf{Title:} Tên chứng nhận (ví dụ: ``ISO 9001:2015''), tối đa 200 ký tự.
    \item \textbf{Subtitle:} Mô tả phụ (tùy chọn), tối đa 200 ký tự.
    \item \textbf{Badge icon:} Dropdown chọn biểu tượng từ thư viện Material Icons (ví dụ: ``Workspace Premium'').
    \item \textbf{Hành động:} Nút Save lưu badge vào danh sách; Cancel đóng modal.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_10_clinic_badges_complete.png}
    \vspace{1em}
    \caption{Complete Clinic Profile -- Gallery và Trust Badges hoàn tất, sẵn sàng đăng}
    \label{fig:partner_clinic_complete}
\end{figure}

Hình~\ref{fig:partner_clinic_complete} hiển thị trạng thái hoàn tất toàn bộ hồ sơ công khai:
\begin{itemize}
    \item \textbf{Clinic gallery:} 3 ảnh đã được sắp xếp với nút mũi tên lên/xuống và xóa.
    \item \textbf{Trust badges:} Badge ``Spa\_trust\_bagde -- Quality Management'' đã được thêm, kèm nút sắp xếp và chỉnh sửa.
    \item \textbf{Nút Complete profile:} ``All required sections are ready. Required completion: 4/4'' -- đối tác nhấn ``Complete profile'' để công khai trang phòng khám.
\end{itemize}
...
Giao diện tạo nhân sự hỗ trợ hai loại hồ sơ chuyên biệt: \textbf{Therapist} (nhà trị liệu) và \textbf{Doctor} (bác sĩ), mỗi loại có các trường thông tin riêng.

\subparagraph{Tạo hồ sơ Nhà trị liệu (Therapist)}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_11_create_therapist_basic.png}
    \vspace{1em}
    \caption{Tạo nhân sự -- Contact \& Basic Info và Professional Role (Therapist)}
    \label{fig:partner_therapist_basic}
\end{figure}

Hình~\ref{fig:partner_therapist_basic} hiển thị phần đầu form tạo Therapist:
\begin{itemize}
    \item \textbf{Contact \& Basic Info:} First Name, Last Name, Email, Phone Number, Date of Birth, Gender, Emergency Contact.
    \item \textbf{Professional Role:} Loại hợp đồng (Full-time/Part-time), ngày bắt đầu, mô tả công việc.
    \item \textbf{Skills \& Services:} Chọn Therapist Type (Spa Therapist / Massage Therapist).
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_12_create_therapist_skills.png}
    \vspace{1em}
    \caption{Tạo nhân sự -- Skills \& Services chi tiết (Therapist)}
    \label{fig:partner_therapist_skills}
\end{figure}

Hình~\ref{fig:partner_therapist_skills} hiển thị các trường chuyên môn của Therapist:
\begin{itemize}
    \item \textbf{Therapist Level:} Cấp độ chuyên môn (Junior/Senior/Expert).
    \item \textbf{Commission Rate:} Tỷ lệ hoa hồng (\%).
    \item \textbf{Massage Strength:} Mức lực (Soft / Medium / Strong).
    \item \textbf{Last Health Check:} Ngày kiểm tra sức khỏe gần nhất; Skill Set: danh sách kỹ năng cụ thể.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_13_create_therapist_history.png}
    \vspace{1em}
    \caption{Tạo nhân sự -- Work History và Documents \& Certifications (Therapist)}
    \label{fig:partner_therapist_history}
\end{figure}

Hình~\ref{fig:partner_therapist_history} hiển thị phần hồ sơ lịch sử và giấy tờ:
\begin{itemize}
    \item \textbf{Work History:} Thêm lịch sử công tác qua nút ``+ Add Work History''.
    \item \textbf{Documents \& Certifications:} Tải lên Professional License/Practice Permit (bắt buộc), Identity Card/Passport (bắt buộc), và Other Documents (tùy chọn).
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_14_create_therapist_schedule.png}
    \vspace{1em}
    \caption{Tạo nhân sự -- Work Schedule (Therapist)}
    \label{fig:partner_therapist_schedule}
\end{figure}

Hình~\ref{fig:partner_therapist_schedule} hiển thị phần lịch làm việc:
\begin{itemize}
    \item \textbf{Work Schedule:} Thiết lập ca làm việc theo từng ngày trong tuần (Mon--Sun), giờ bắt đầu/kết thúc ca.
    \item \textbf{Default Shift Pattern:} Chọn mẫu ca mặc định (Full-Time Morning/Afternoon/Full, hoặc Part-Time).
\end{itemize}

\subparagraph{Tạo hồ sơ Bác sĩ (Doctor)}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{Images/Hiện thực/partner/fig_partner_15_create_doctor_education.png}
    \vspace{1em}
    \caption{Tạo nhân sự -- Education \& Qualifications và Experience (Doctor)}
    \label{fig:partner_doctor_edu}
\end{figure}

Hình~\ref{fig:partner_doctor_edu} hiển thị các trường đặc thù của Doctor (ngoài Contact \& Basic Info giống Therapist):
\begin{itemize}
    \item \textbf{Education \& Qualifications:} Medical Title (học hàm/học vị), Medical License ID (số giấy phép hành nghề), Education (bằng cấp), Certifications (chứng chỉ chuyên môn).
    \item \textbf{Experience \& Qualifications:} Years of Experience, Consultation Fee (\%), Specializations (chuyên khoa).
\end{itemize}

\subparagraph{Hồ sơ Nhân sự đã hoàn thiện}

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{Images/Hiện thực/partner/fig_partner_17_therapist_form_filled.png}
    \caption{Hồ sơ Therapist đã điền đầy đủ}
    \label{fig:partner_therapist_filled}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{Images/Hiện thực/partner/fig_partner_18_doctor_form_filled.png}
    \caption{Hồ sơ Doctor đã điền đầy đủ}
    \label{fig:partner_doctor_filled}
\end{subfigure}
\par\medskip
\caption{Form tạo nhân sự hoàn chỉnh -- Therapist và Doctor}
\label{fig:partner_staff_filled}
\end{figure}

Hình~\ref{fig:partner_staff_filled} minh họa form nhân sự đã điền đủ thông tin:
\begin{itemize}
    \item \textbf{Profile image:} Ảnh đại diện của nhân sự (hình vuông) đã tải lên.
    \item \textbf{Tab phân loại:} Tab ``Therapist'' hoặc ``Doctor'' được chọn, xác định loại hồ sơ và các trường đặc thù.
    \item \textbf{Thông tin liên hệ:} Họ tên, email, số điện thoại, loại hợp đồng.
    \item \textbf{Employee ID:} Mã nhân viên tự động sinh (ví dụ: \texttt{C0CA87AE}).
    \item \textbf{Nút lưu:} ``Create Therapist'' hoặc ``Create Doctor'' để lưu hồ sơ vào hệ thống.
\end{itemize}
...
\begin{figure}[H]
    ...
    \vspace{1em}
    \caption{Xác nhận đăng ký đối tác thành công -- Hồ sơ đang chờ xét duyệt}
    \label{fig:admin_partner_reg_success}
\end{figure}

Hình~\ref{fig:admin_partner_reg_success} hiển thị trang xác nhận sau khi đối tác nộp đầy đủ hồ sơ:
\begin{itemize}
