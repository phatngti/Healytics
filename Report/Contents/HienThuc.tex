...
Module Employee cho phép đối tác quản lý hồ sơ nhân sự (Therapist, Doctor\ldots), xem trạng thái và tạo mới nhân viên với thông tin chuyên môn đầy đủ.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Images/Hiện thực/partner/employee/fig_partner_employee_1.png}
    \vspace{0.5em}
    \caption{Employee -- danh sách nhân viên dạng lưới thẻ}
    \label{fig:partner_emp1}
\end{figure}

Hình~\ref{fig:partner_emp1} hiển thị danh sách nhân viên dạng card grid:
\begin{itemize}
    \item Mỗi thẻ (Card): ảnh đại diện, tên, vai trò (Therapist/Doctor), badge trạng thái, nút xem chi tiết.
    \item Hỗ trợ tìm kiếm và lọc theo vai trò hoặc trạng thái.
\end{itemize}
...
\begin{figure}[H]
    ...
    \vspace{0.5em}
    \caption{Employee -- danh sách nhân viên dạng bảng với ID, vai trò, giờ làm và thao tác}
    \label{fig:partner_emp2}
\end{figure}

Hình~\ref{fig:partner_emp2} hiển thị danh sách nhân viên dạng bảng:
\begin{itemize}
    \item Cột: Employee ID (Mã nhân viên), Name (Tên), Role (Vai trò), Working Hours (Giờ làm việc), Status (Trạng thái), 3 nút thao tác (xem/sửa/xóa).
    \item Thuận tiện cho thao tác hàng loạt.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Images/Hiện thực/partner/employee/fig_partner_employee_2_b.png}
    \vspace{0.5em}
    \caption{Employee -- bộ lọc nâng cao theo vai trò, trạng thái và từ khóa}
    \label{fig:partner_emp2b}
\end{figure}

Hình~\ref{fig:partner_emp2b} cho thấy bộ lọc nâng cao:
\begin{itemize}
    \item Lọc theo Role (Vai trò), Status (Trạng thái: Active -- Đang làm/Inactive -- Ngừng hoạt động/On Leave -- Nghỉ phép) và từ khóa tìm kiếm.
    \item Kết quả cập nhật realtime.
\end{itemize}
...
\begin{figure}[H]
    ...
    \vspace{0.5em}
    \caption{Chi tiết nhân viên -- hồ sơ đầy đủ tổ chức theo nhiều tab}
    \label{fig:partner_emp3}
\end{figure}

Hình~\ref{fig:partner_emp3} hiển thị trang chi tiết nhân viên:
\begin{itemize}
    \item Các tab: Contact \& Basic Info, Professional Role, Skills \& Services, Work Schedule, Work History, Documents.
    \item Mỗi tab có thể cập nhật độc lập.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Images/Hiện thực/partner/employee/fig_partner_employee_4.png}
    \vspace{0.5em}
    \caption{Chi tiết nhân viên -- tab Work Schedule: ca làm việc theo từng ngày trong tuần}
    \label{fig:partner_emp4}
\end{figure}

Hình~\ref{fig:partner_emp4} là tab Work Schedule:
\begin{itemize}
    \item Cấu hình ca theo từng ngày (Mon--Sun): giờ bắt đầu/kết thúc và toggle bật/tắt.
    \item Dữ liệu dùng bởi BookingModule để kiểm tra khả dụng khi đặt lịch.
\end{itemize}
...
\begin{figure}[H]
    ...
    \vspace{0.5em}
    \caption{Tạo nhân viên -- Contact \& Basic Info và chọn Employee Type}
    \label{fig:partner_create_emp1}
\end{figure}

Hình~\ref{fig:partner_create_emp1} hiển thị bước đầu của form tạo nhân viên:
\begin{itemize}
    \item Tab chọn loại: \textbf{Therapist} hoặc \textbf{Doctor} -- ảnh hưởng đến các trường chuyên môn hiển thị.
    \item \textbf{Contact \& Basic Info} (Thông tin liên hệ cơ bản): First Name (Tên), Last Name (Họ), Email, Phone (Số điện thoại), Date of Birth (Ngày sinh), Gender (Giới tính).
\end{itemize}
...
\begin{figure}[H]
    ...
    \vspace{0.5em}
    \caption{Tạo nhân viên -- Skills \& Services: cấp bậc, hoa hồng và kỹ năng chuyên môn}
    \label{fig:partner_create_emp2}
\end{figure}

    Hình~\ref{fig:partner_create_emp2} hiển thị mục \textbf{Skills \& Services} cho Doctor:
\begin{itemize}
    \item \textbf{Doctor Level:} Cấp bậc chuyên môn (Junior/Senior/Expert); \textbf{Commission Rate:} Tỷ lệ hoa hồng (\%).
    \item \textbf{Specialization:} Chuyên môn chính của bác sĩ.
\end{itemize}
...
\begin{figure}[H]
    ...
    \vspace{0.5em}
    \caption{Tạo nhân viên -- Skills \& Services: chọn Therapist Type}
    \label{fig:partner_create_emp2b}
\end{figure}

Hình~\ref{fig:partner_create_emp2b} hiển thị mục \textbf{Skills \& Services}  cho Therapist:
\begin{itemize}
    \item \textbf{Therapist Type:} Chọn loại trị liệu viên -- Spa Therapist (wellness tổng quát) hoặc Massage Therapist (deep tissue).
    \item \textbf{Last Health Check:} Ngày kiểm tra sức khoẻ gần nhất.
    \item \textbf{Skill Set:} Tìm kiếm và gắn các kỹ năng chuyên môn (đa chọn).
\end{itemize}
...
\begin{figure}[H]
    ...
    \vspace{0.5em}
    \caption{Tạo nhân viên -- Work History, Documents \& Certifications và Work Schedule}
    \label{fig:partner_create_emp3}
\end{figure}

Hình~\ref{fig:partner_create_emp3} hiển thị phần cuối của form tạo nhân viên:
\begin{itemize}
    \item \textbf{Work History:} Thêm nhiều mục lịch sử công tác qua nút \textbf{Add Work History}.
    \item \textbf{Documents \& Certifications -- Required:} Giấy phép hành nghề (License/Permit) và Chứng minh nhân thân (Identity Card) -- bắt buộc.
    \item \textbf{Other Documents:} Khu vực tải lên tự do (PDF, JPG, PNG, tối đa 10MB).
    \item \textbf{Work Schedule:} Cấu hình ca làm việc theo từng ngày trong tuần; nhấn \textbf{Create Employee/Doctor} để lưu và hệ thống sinh Employee ID tự động.
\end{itemize}
