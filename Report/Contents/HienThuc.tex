...
\subsection{Quản lý Dịch vụ và Danh mục}

\subsubsection{Module Dịch vụ Sức khỏe (\texttt{HealthServiceModule})}

Module nghiệp vụ cốt lõi, quản lý vòng đời dịch vụ sức khỏe trên nền tảng. Hai controller tách biệt hai phía truy cập:

\begin{itemize}
    \item \textbf{\texttt{UserHealthServiceController}:} Tìm kiếm, lọc, xem chi tiết dịch vụ. Hỗ trợ phân trang, sắp xếp (giá, đánh giá, khoảng cách) và lọc theo danh mục hoặc tags.
    \item \textbf{\texttt{PartnerHealthServiceController}:} CRUD dịch vụ, quản lý trạng thái hiển thị (ẩn/hiện). Đối tác chỉ thao tác trên dịch vụ thuộc sở hữu của mình.
\end{itemize}
...
% TODO: Thêm hình minh họa mô hình dữ liệu Product và các quan hệ

\subsubsection{Module Danh mục (\texttt{CategoriesModule})}

Tổ chức danh mục theo cấu trúc phân cấp parent-child (ví dụ: Y tế $\rightarrow$ Tim mạch $\rightarrow$ Khám tổng quát). Ba controller phân quyền rõ ràng:

\begin{itemize}
    \item \texttt{AdminCategoriesController}: CRUD danh mục (chỉ admin).
    \item \texttt{UserCategoriesController}: Lấy danh sách kèm bộ lọc cho ứng dụng.
    \item \texttt{CategoriesController}: API công khai không yêu cầu xác thực.
\end{itemize}

\subsubsection{Module Tags dịch vụ (\texttt{ServiceTagsModule})}

Quản lý hệ thống tags gắn với dịch vụ (\textit{yoga}, \textit{giảm cân}, \textit{tim mạch}). Quan hệ many-to-many với \texttt{Product}, hỗ trợ tìm kiếm đa chiều. API cho phép CRUD tags và tìm dịch vụ theo tổ hợp tags.

\subsection{Quản lý Nhân sự và Cơ sở}

\subsubsection{Module Nhân viên (\texttt{EmployeesModule})}
...
\begin{itemize}
    ...
    \item \textbf{Hồ sơ chuyên biệt:} Mở rộng từ \texttt{Employee} thành \texttt{DoctorProfile} (chuyên khoa, giấy phép hành nghề) và \texttt{TherapistProfile} (phương pháp trị liệu, kinh nghiệm).
\end{itemize}

\subsubsection{Module Phòng khám (\texttt{ClinicModule})}

Quản lý phòng khám và cơ sở dịch vụ: tên, mô tả, hình ảnh, giờ hoạt động. Liên kết \texttt{Address} và tọa độ địa lý phục vụ tìm kiếm theo vị trí. \texttt{UserClinicController} cho phép tìm phòng khám gần vị trí hiện tại, lọc theo dịch vụ hoặc danh mục.

\subsection{Quản lý Đặt lịch, Giỏ hàng và Thanh toán}

\subsubsection{Module Đặt lịch (\texttt{BookingModule})}

Xử lý quy trình đặt lịch theo mô hình máy trạng thái:

\begin{itemize}
    \item \textbf{Luồng trạng thái:} \texttt{PENDING} $\rightarrow$ \texttt{CONFIRMED} $\rightarrow$ \texttt{IN\_PROGRESS} $\rightarrow$ \texttt{COMPLETED} hoặc \texttt{CANCELLED}. Mỗi chuyển trạng thái được ghi vào \texttt{BookingStatusLog}.
...
% TODO: Thêm hình minh họa state machine của Booking

\subsubsection{Module Lịch hẹn (\texttt{AppointmentModule})}
...
\subsubsection{Module Giỏ hàng (\texttt{CartModule})}
...
\subsubsection{Module Thanh toán (\texttt{PaymentGatewayModule})}

Tích hợp thanh toán trực tuyến qua cổng \textbf{MoMo}:

\begin{itemize}
    \item \textbf{Tạo giao dịch:} \texttt{MoMoPaymentService} tạo request, ký HMAC-SHA256, gửi đến MoMo API.
    \item \textbf{Xử lý callback (IPN):} \texttt{MoMoController} nhận kết quả từ MoMo, xác thực chữ ký.
    \item \textbf{Liên kết Booking:} \texttt{BookingPaymentService} cập nhật trạng thái (thành công $\rightarrow$ \texttt{CONFIRMED}, thất bại $\rightarrow$ \texttt{PAYMENT\_FAILED}).
    \item Giao dịch lưu trong \texttt{Payment} và \texttt{PaymentTransactionLog}.
\end{itemize}

% TODO: Thêm hình minh họa luồng thanh toán MoMo (sequence diagram)

\subsection{Hệ thống Giao tiếp và Thông báo}

\subsubsection{Module Chat thời gian thực (\texttt{ChatModule})}

Nhắn tin thời gian thực giữa người dùng và đối tác, kết hợp REST API và WebSocket:

\begin{itemize}
    \item \textbf{Kiến trúc song song:}
    \begin{itemize}
        \item Phía người dùng: \texttt{UserChatController} (REST) + \texttt{UserChatGateway} (WebSocket).
        \item Phía đối tác: \texttt{PartnerChatController} (REST) + \texttt{PartnerChatGateway} (WebSocket).
    \end{itemize}
    \item \textbf{Xác thực WebSocket:} Middleware \texttt{ws-jwt-auth} xác thực JWT khi thiết lập kết nối.
    \item \textbf{Dữ liệu:} \texttt{Conversation}/\texttt{ChatMessage} (nội dung, thời gian, trạng thái đã đọc), \texttt{PartnerChatAttachment} (tệp đính kèm).
    \item \textbf{REST bổ trợ:} Lịch sử tin nhắn (phân trang), danh sách cuộc trò chuyện, đánh dấu đã đọc.
\end{itemize}

\subsubsection{Module Thông báo (\texttt{NotificationModule})}

Thông báo đa kênh:

\begin{itemize}
    \item \textbf{WebSocket:} \texttt{NotificationGateway} đẩy thông báo tức thì (đặt lịch, tin nhắn mới, nhắc hẹn).
    \item \textbf{Push Notification (FCM):} \texttt{PushNotificationService} tích hợp Firebase Cloud Messaging. \texttt{UserDeviceController} quản lý đăng ký device token.
    ...
    \item \textbf{Bất đồng bộ:} Sự kiện thông báo phát qua RabbitMQ, không làm chậm luồng nghiệp vụ chính.
\end{itemize}

\subsection{Hệ thống Hỗ trợ Nghiệp vụ}

\subsubsection{Module Địa chỉ hành chính (\texttt{LocationsModule})}
...
\subsubsection{Module Bản đồ (\texttt{MapboxModule})}
...
\subsubsection{Module Đánh giá (\texttt{ReviewModule})}

Cho phép đánh giá sau khi sử dụng dịch vụ. Hai loại: \texttt{SpecialistReview} (đánh giá chuyên gia về chuyên môn, thái độ) và \texttt{TreatmentReview} (đánh giá trải nghiệm điều trị, cơ sở vật chất). Điểm trung bình được tổng hợp và hiển thị trên trang chi tiết.

\subsection{Dashboard Đối tác và Tích hợp AI}
...
% TODO: Thêm hình minh họa giao diện dashboard đối tác
...
\subsection{Hạ tầng Liên kết và Cross-cutting Concerns}

\subsubsection{Module RabbitMQ}
...
\subsubsection{Middleware, Interceptor và Guard}
