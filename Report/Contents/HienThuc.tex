...
\subsection{Quản lý Dịch vụ Sức khỏe (Health Service Management)}

Module \texttt{HealthServiceModule} là module nghiệp vụ cốt lõi, quản lý toàn bộ vòng đời của các dịch vụ sức khỏe (sản phẩm) trên nền tảng. Hệ thống phân tách rõ ràng giữa hai luồng truy cập thông qua hai controller riêng biệt:

\begin{itemize}
    \item \textbf{Controller phía người dùng (\texttt{UserHealthServiceController}):} Cung cấp các API cho phép người dùng cuối tìm kiếm, lọc và xem chi tiết dịch vụ. Hỗ trợ phân trang, sắp xếp theo nhiều tiêu chí (giá, đánh giá, khoảng cách), và lọc theo danh mục hoặc tags.

    \item \textbf{Controller phía đối tác (\texttt{PartnerHealthServiceController}):} Cho phép đối tác tạo mới, cập nhật, xóa và quản lý trạng thái hiển thị (ẩn/hiện) của dịch vụ. Mỗi đối tác chỉ có thể thao tác trên các dịch vụ thuộc sở hữu của mình.

    \item \textbf{Mô hình dữ liệu:} Mỗi dịch vụ (\texttt{Product}) liên kết với nhiều thực thể con:
    \begin{itemize}
        \item \texttt{ProductMedia}: Hình ảnh và video giới thiệu dịch vụ.
        \item \texttt{ProductFacilityImage}: Ảnh cơ sở vật chất nơi cung cấp dịch vụ.
        \item \texttt{ProductTag} và \texttt{ProductFeatureTag}: Hệ thống tags đa tầng phục vụ tìm kiếm và phân loại.
        \item \texttt{ProductEmployeeEligibility}: Danh sách chuyên gia đủ điều kiện thực hiện dịch vụ.
        \item \texttt{ProductResourceRequirement}: Yêu cầu về trang thiết bị, phòng ốc.
        \item \texttt{DocumentRequirement}: Giấy tờ cần thiết từ phía bệnh nhân/khách hàng.
    \end{itemize}
\end{itemize}

\subsection{Quản lý Danh mục và Tags (Categories \& Service Tags)}

\subsubsection{Danh mục dịch vụ (\texttt{CategoriesModule})}

Hệ thống danh mục được tổ chức theo cấu trúc phân cấp (parent-child), cho phép phân loại dịch vụ theo nhiều cấp độ (ví dụ: Y tế → Tim mạch → Khám tổng quát). Module cung cấp ba controller riêng biệt theo vai trò:

\begin{itemize}
    \item \texttt{AdminCategoriesController}: Tạo, sửa, xóa danh mục (chỉ dành cho quản trị viên).
    \item \texttt{UserCategoriesController}: Lấy danh sách danh mục kèm bộ lọc, phục vụ hiển thị trên ứng dụng.
    \item \texttt{CategoriesController}: API công khai cho các chức năng không yêu cầu xác thực.
\end{itemize}

\subsubsection{Tags dịch vụ (\texttt{ServiceTagsModule})}

Module quản lý hệ thống tags gắn với dịch vụ (ví dụ: ``yoga'', ``giảm cân'', ``tim mạch''), hỗ trợ tìm kiếm đa chiều. Tags được tổ chức theo quan hệ many-to-many với \texttt{Product}, cho phép một dịch vụ có nhiều tags và ngược lại. API hỗ trợ tạo, cập nhật, xóa tags và tìm kiếm dịch vụ theo tổ hợp tags.

\subsection{Quản lý Nhân viên và Chuyên gia (Employee Management)}

Module \texttt{EmployeesModule} quản lý thông tin nhân viên, bác sĩ, và các chuyên gia y tế/thể thao/dinh dưỡng trên nền tảng. Đây là module phức tạp với logic nghiệp vụ dày đặc (service 19KB):

\begin{itemize}
    \item \textbf{Phân quyền Controller:}
    \begin{itemize}
        \item \texttt{PartnerEmployeesController}: Cho phép đối tác quản lý (CRUD) nhân viên thuộc tổ chức mình, bao gồm cập nhật lịch làm việc, chuyên môn, và trạng thái hoạt động.
        \item \texttt{UserEmployeesController}: Cho phép người dùng cuối xem thông tin chuyên gia, tìm kiếm theo chuyên khoa, và xem lịch trống.
    \end{itemize}

    \item \textbf{Hồ sơ chuyên biệt:} Hệ thống hỗ trợ hai loại hồ sơ chuyên gia được mở rộng từ thực thể \texttt{Employee} cơ bản:
    \begin{itemize}
        \item \texttt{DoctorProfile}: Chứa thông tin chuyên khoa, số giấy phép hành nghề, chứng chỉ.
        \item \texttt{TherapistProfile}: Chứa thông tin về phương pháp trị liệu, lĩnh vực chuyên môn, kinh nghiệm.
    \end{itemize}
\end{itemize}

\subsection{Quản lý Phòng khám và Cơ sở (Clinic Management)}

Module \texttt{ClinicModule} quản lý thông tin phòng khám, trung tâm y tế, và các cơ sở cung cấp dịch vụ. Service (21KB) xử lý:

\begin{itemize}
    \item Quản lý thông tin cơ sở: tên, mô tả, hình ảnh, giờ hoạt động.
    \item Liên kết với hệ thống địa chỉ (\texttt{Address}) và tọa độ địa lý để hỗ trợ tìm kiếm theo vị trí.
    \item \texttt{UserClinicController} cung cấp API cho người dùng tìm kiếm phòng khám gần vị trí hiện tại, lọc theo dịch vụ hoặc danh mục.
\end{itemize}

\subsection{Quản lý Đặt lịch và Lịch hẹn (Booking \& Appointment)}

\subsubsection{Đặt lịch (\texttt{BookingModule})}

Module xử lý toàn bộ quy trình đặt lịch dịch vụ sức khỏe, hoạt động theo mô hình máy trạng thái:

\begin{itemize}
    \item \textbf{Luồng trạng thái:} \texttt{PENDING} → \texttt{CONFIRMED} → \texttt{IN\_PROGRESS} → \texttt{COMPLETED} hoặc \texttt{CANCELLED}. Mỗi lần chuyển trạng thái đều được ghi lại trong bảng \texttt{BookingStatusLog} để đảm bảo truy vết.

    \item \textbf{Quản lý slot thời gian:} \texttt{SlotsController} cung cấp API kiểm tra và quản lý các khung giờ trống, ngăn chặn tình trạng đặt trùng (double-booking) thông qua cơ chế khóa phân tán Redis.

    \item \textbf{Tích hợp thanh toán:} Sau khi booking được xác nhận, hệ thống tự động kích hoạt quy trình thanh toán thông qua \texttt{BookingPaymentService}.
\end{itemize}

\subsubsection{Lịch hẹn (\texttt{AppointmentModule})}

Module quản lý chi tiết lịch hẹn sau khi booking đã được xác nhận:

\begin{itemize}
    \item \texttt{UserAppointmentController}: Cho phép người dùng xem danh sách lịch hẹn sắp tới, hủy hoặc đổi lịch.
    \item Mỗi appointment liên kết 1-1 với booking gốc, đảm bảo tính nhất quán dữ liệu.
    \item Hỗ trợ nhắc nhở tự động thông qua tích hợp với \texttt{NotificationModule}.
\end{itemize}

\subsection{Giỏ hàng (Cart Management)}

Module \texttt{CartModule} quản lý giỏ hàng với mô hình checkout-ticket:

\begin{itemize}
    \item \textbf{Thêm/xóa dịch vụ:} API cho phép thêm, xóa, hoặc cập nhật số lượng dịch vụ trong giỏ.
    \item \textbf{Checkout Ticket:} Khi người dùng tiến hành thanh toán, hệ thống tạo \texttt{CheckoutTicket} chứa toàn bộ thông tin đơn hàng (dịch vụ, giá, mã giảm giá) tại thời điểm checkout, đảm bảo giá không thay đổi trong quá trình thanh toán.
    \item \textbf{Giới hạn:} Hệ thống áp dụng các ràng buộc (constants) về số lượng tối đa trong giỏ và thời gian hết hạn của checkout ticket.
\end{itemize}

\subsection{Cổng Thanh toán (Payment Gateway)}

Module \texttt{PaymentGatewayModule} tích hợp thanh toán trực tuyến, hiện hỗ trợ cổng thanh toán \textbf{MoMo}:

\begin{itemize}
    \item \textbf{Tạo giao dịch (\texttt{MoMoPaymentService}):} Khi người dùng chọn thanh toán qua MoMo, hệ thống tạo request chứa thông tin đơn hàng, ký HMAC-SHA256, và gửi đến MoMo API. Người dùng được chuyển hướng đến trang thanh toán MoMo.

    \item \textbf{Xử lý callback (IPN):} \texttt{MoMoController} cung cấp webhook endpoint nhận thông báo kết quả thanh toán từ MoMo (Instant Payment Notification). Hệ thống xác thực chữ ký (signature verification) để đảm bảo request hợp lệ.

    \item \textbf{Liên kết với Booking:} \texttt{BookingPaymentService} đóng vai trò cầu nối, tự động cập nhật trạng thái booking sau khi nhận kết quả thanh toán (thành công → \texttt{CONFIRMED}, thất bại → \texttt{PAYMENT\_FAILED}).

    \item \textbf{Ghi nhận giao dịch:} Mọi giao dịch đều được lưu trong thực thể \texttt{Payment} và \texttt{PaymentTransactionLog}, ghi nhận chi tiết về số tiền, phương thức, mã giao dịch, và trạng thái.
\end{itemize}

\subsection{Hệ thống Chat thời gian thực (Real-time Chat)}

Module \texttt{ChatModule} triển khai hệ thống nhắn tin thời gian thực giữa người dùng và đối tác (phòng khám/chuyên gia), sử dụng kết hợp REST API và WebSocket:

\begin{itemize}
    \item \textbf{Kiến trúc song song:} Hệ thống tách biệt hoàn toàn hai phía giao tiếp:
    \begin{itemize}
        \item \textit{Phía người dùng:} \texttt{UserChatController} (REST) + \texttt{UserChatGateway} (WebSocket).
        \item \textit{Phía đối tác:} \texttt{PartnerChatController} (REST) + \texttt{PartnerChatGateway} (WebSocket).
    \end{itemize}

    \item \textbf{WebSocket Authentication:} Middleware \texttt{ws-jwt-auth.middleware.ts} xác thực JWT token khi client thiết lập kết nối WebSocket, đảm bảo chỉ người dùng hợp lệ mới có thể tham gia chat.

    \item \textbf{Mô hình dữ liệu:}
    \begin{itemize}
        \item \texttt{Conversation}/\texttt{PartnerConversation}: Quản lý cuộc trò chuyện, bao gồm thông tin người tham gia và trạng thái.
        \item \texttt{ChatMessage}/\texttt{PartnerChatMessage}: Lưu trữ nội dung tin nhắn, thời gian gửi, trạng thái đã đọc.
        \item \texttt{PartnerChatAttachment}: Hỗ trợ gửi tệp đính kèm (hình ảnh, tài liệu y tế).
    \end{itemize}

    \item \textbf{REST API bổ trợ:} Ngoài WebSocket cho giao tiếp thời gian thực, REST API cung cấp các chức năng như: lấy lịch sử tin nhắn (có phân trang), danh sách cuộc trò chuyện, đánh dấu đã đọc.
\end{itemize}

\subsection{Hệ thống Thông báo (Notification System)}

Module \texttt{NotificationModule} quản lý toàn bộ hệ thống thông báo đa kênh, bao gồm thông báo trong ứng dụng (in-app), thông báo thời gian thực (WebSocket), và thông báo đẩy (push notification):

\begin{itemize}
    \item \textbf{Thông báo thời gian thực:} \texttt{NotificationGateway} (WebSocket) đẩy thông báo tức thì đến client khi có sự kiện mới (đặt lịch thành công, tin nhắn mới, nhắc hẹn).

    \item \textbf{Push Notification (FCM):} \texttt{PushNotificationService} tích hợp Firebase Cloud Messaging để gửi thông báo đẩy đến thiết bị di động ngay cả khi ứng dụng không mở.
    \begin{itemize}
        \item \texttt{UserDeviceController}: API đăng ký/hủy đăng ký device token từ client.
        \item Thực thể \texttt{DeviceToken} lưu token FCM của từng thiết bị, liên kết với tài khoản người dùng.
    \end{itemize}

    \item \textbf{Quản lý thông báo:}
    \begin{itemize}
        \item \texttt{UserNotificationController}: Lấy danh sách thông báo, đánh dấu đã đọc, xóa thông báo.
        \item \texttt{AdminNotificationController}: Gửi thông báo broadcast đến nhóm người dùng hoặc toàn bộ hệ thống.
        \item Thực thể \texttt{NotificationRead} theo dõi trạng thái đã đọc cho từng người dùng, hỗ trợ badge count.
    \end{itemize}

    \item \textbf{Tích hợp RabbitMQ:} Các sự kiện thông báo được phát qua message queue, đảm bảo tính bất đồng bộ và không làm chậm luồng nghiệp vụ chính.
\end{itemize}

\subsection{Hệ thống Địa lý và Bản đồ (Location \& Map Services)}

\subsubsection{Quản lý địa chỉ hành chính (\texttt{LocationsModule})}

Module quản lý hệ thống địa chỉ phân cấp theo chuẩn hành chính Việt Nam:

\begin{itemize}
    \item Cấu trúc phân cấp: Tỉnh/Thành phố → Quận/Huyện → Phường/Xã, được định nghĩa trong \texttt{LocationLevel} enum.
    \item API tìm kiếm theo cấp: lấy danh sách tỉnh, quận thuộc tỉnh, xã thuộc quận.
    \item Phục vụ validation địa chỉ khi đăng ký đối tác, tạo phòng khám, và lưu địa chỉ người dùng.
\end{itemize}

\subsubsection{Tích hợp bản đồ (\texttt{MapboxModule})}

Module tích hợp Mapbox API để cung cấp các tính năng địa lý nâng cao:

\begin{itemize}
    \item \textbf{Geocoding:} Chuyển đổi địa chỉ văn bản thành tọa độ (latitude/longitude) và ngược lại (reverse geocoding).
    \item \textbf{Tìm kiếm theo vị trí:} Cho phép người dùng tìm phòng khám, chuyên gia gần vị trí hiện tại với bán kính tùy chỉnh.
    \item \textbf{Tính khoảng cách:} Hỗ trợ tính khoảng cách giữa hai điểm, phục vụ sắp xếp kết quả tìm kiếm theo độ gần.
\end{itemize}

\subsection{Hệ thống Đánh giá (Review System)}

Module \texttt{ReviewModule} cho phép người dùng đánh giá dịch vụ và chuyên gia sau khi sử dụng:

\begin{itemize}
    \item \texttt{UserReviewController}: API gửi đánh giá, xem đánh giá của mình, và xem đánh giá từ người dùng khác.
    \item \textbf{Hai loại đánh giá:}
    \begin{itemize}
        \item \texttt{SpecialistReview}: Đánh giá chuyên gia (bác sĩ, huấn luyện viên) về chuyên môn, thái độ, và hiệu quả.
        \item \texttt{TreatmentReview}: Đánh giá trải nghiệm điều trị, bao gồm cơ sở vật chất, quy trình, và kết quả.
    \end{itemize}
    \item Điểm đánh giá trung bình được tổng hợp và hiển thị trên trang chi tiết dịch vụ và hồ sơ chuyên gia.
\end{itemize}

\subsection{Tích hợp cross-cutting: Middleware, Interceptor và Guard}

Ngoài các module nghiệp vụ, hệ thống triển khai các cơ chế xuyên suốt (cross-cutting concerns) để đảm bảo chất lượng vận hành:

\begin{itemize}
    \item \textbf{LoggingMiddleware:} Được áp dụng toàn cục trên mọi route, ghi nhận method, URL, thời gian phản hồi, và status code của mỗi request, phục vụ giám sát và debug.

    \item \textbf{ResponseInterceptor:} Chuẩn hóa toàn bộ response API về một format thống nhất bao gồm \texttt{statusCode}, \texttt{message}, và \texttt{data}, giúp phía Client xử lý dữ liệu nhất quán.

    \item \textbf{PublicThrottlerGuard:} Áp dụng rate limiting 100 request/60 giây cho các route công khai, ngăn chặn tấn công brute-force và DDoS cơ bản.

    \item \textbf{WebSocket Contract Bootstrap:} Service \texttt{WsContractBootstrapService} tự động đăng ký và xác thực các WebSocket event contracts khi ứng dụng khởi động, đảm bảo tính nhất quán giữa client và server.
\end{itemize}
