...
class DashboardNotifier extends _$DashboardNotifier {
  ...
  FutureOr<DashboardState> build() =>
      _fetchAll(DashboardTimePeriod.thisMonth);
  ...
  Future<DashboardState> _fetchAll(
    ...
    final results = await Future.wait([
      _safeCall(() => repo.getDashboardStats(
            period: period,
          )),
      _safeCall(() => repo.getRevenueData(period)),
      _safeCall(
        () => repo.getUpcomingAppointments(),
      ),
      _safeCall(
        () => repo.getServicePerformance(),
      ),
      _safeCall(
        () => repo.getEmployeeDistribution(),
      ),
      _safeCall(() => repo.getRecentReviews()),
      _safeCall(
        () => repo.getStaffSchedule(DateTime.now()),
      ),
    ...
    return DashboardState(
      stats: results[0] as DashboardStats? ??
          _emptyStats,
      revenueData:
          results[1] as List<RevenueDataPoint>? ??
              [],
      upcomingAppointments: results[2]
              as List<UpcomingAppointment>? ??
          [],
      servicePerformance: results[3]
              as List<ServicePerformance>? ??
          [],
      employeeDistribution: results[4]
              as List<EmployeeDistribution>? ??
          [],
      recentReviews:
          results[5] as List<DashboardReview>? ?? [],
      staffSchedule: results[6]
              as List<StaffScheduleEntry>? ??
          [],
      notifications: results[7]
              as List<DashboardNotification>? ??
          [],
      inventoryAlerts:
          results[8] as List<InventoryAlert>? ?? [],
  ...
  Future<void> refresh() async {
    ...
    final period = previous?.selectedPeriod ??
  ...
  Future<void> setTimePeriod(
    ...
    try {
      ...
      final newRevenue = await _safeCall(
        () => repo.getRevenueData(period),
      );
      ...
      state = AsyncData(
        current.copyWith(
          ...
          revenueData:
              newRevenue ?? current.revenueData,
          isRefreshing: false,
        ),
      );
    } catch (e, st) {
