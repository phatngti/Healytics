...
/// Contract for employee analytics remote data
/// operations.
...
/// Real implementation using the Partner Employees
/// Analytics API.
class EmployeeAnalyticsRemoteDataSourceImpl
  ...
  Future<EmployeeOverviewAnalytics> getOverviewAnalytics({
    ...
    if (response == null) {
      throw const EmployeeAnalyticsDataException(
  ...
  // ── Domain → Generated Enum Converter ─────────

  /// Converts the domain [DashboardTimePeriod] enum
  /// to the generated OpenAPI [DashboardTimePeriod]
  /// class required by the API client.
  openapi.DashboardTimePeriod _toApiPeriod(
    DashboardTimePeriod period,
  ) {
    return switch (period) {
      DashboardTimePeriod.today =>
        openapi.DashboardTimePeriod.today,
      DashboardTimePeriod.thisWeek =>
        openapi.DashboardTimePeriod.thisWeek,
      DashboardTimePeriod.thisMonth =>
        openapi.DashboardTimePeriod.thisMonth,
      DashboardTimePeriod.thisQuarter =>
        openapi.DashboardTimePeriod.thisQuarter,
      DashboardTimePeriod.thisYear =>
        openapi.DashboardTimePeriod.thisYear,
    };
  }
  ...
  EmployeeOverviewAnalytics _mapOverviewResponse(
    ...
    return EmployeeOverviewAnalytics(
      ...
      roleDistribution: dto.roleDistribution
          .map(_mapRoleDistribution)
          .toList(),
      topPerformers: dto.topPerformers
          .map(_mapPerformanceSummary)
          .toList(),
      complianceItems: dto.complianceItems
          .map(_mapComplianceItem)
          .toList(),
    );
  }
  ...
  EmployeeDetailAnalytics _mapDetailResponse(
    ...
    return EmployeeDetailAnalytics(
      ...
      completedSessions:
          dto.completedSessions.toInt(),
      sessionsDelta: dto.sessionsDelta.toDouble(),
      contributionValue:
          dto.contributionValue.toDouble(),
      contributionDelta:
          dto.contributionDelta.toDouble(),
      utilizationRate:
          dto.utilizationRate.toDouble(),
      utilizationDelta:
          dto.utilizationDelta.toDouble(),
      averageRating: dto.averageRating.toDouble(),
      reviewCount: dto.reviewCount.toInt(),
      trendPoints: dto.trendPoints
          .map(_mapTrendPoint)
          .toList(),
      mixMetrics: dto.mixMetrics
          .map(_mapMixMetric)
          .toList(),
      scheduleLoad: dto.scheduleLoad
          .map(_mapScheduleLoad)
          .toList(),
      qualityMetrics: dto.qualityMetrics
          .map(_mapQualityMetric)
          .toList(),
      complianceItems: dto.complianceItems
          .map(_mapComplianceItem)
          .toList(),
  ...
  EmployeeTrendPoint _mapTrendPoint(
    EmployeeTrendPointDto dto,
  ) {
  ...
  EmployeeMixMetric _mapMixMetric(
    EmployeeMixMetricDto dto,
  ) {
...
/// Uses synthetic data built from employee mock
/// entities.
class EmployeeAnalyticsRemoteDataSourceMock
  ...
  Future<EmployeeOverviewAnalytics> getOverviewAnalytics({
    ...
    final employees = <EmployeeEntity>[
      createMockDoctor(
        const EmployeeId('mock-doc-0'),
      ),
      createMockDoctor(
        const EmployeeId('mock-doc-1'),
      ),
      createMockSpaTherapist(
        const EmployeeId('mock-spa-0'),
      ),
      createMockSpaTherapist(
        const EmployeeId('mock-spa-1'),
      ),
      createMockMassageTherapist(
        const EmployeeId('mock-massage-0'),
      ),
    ];
    return _buildEmployeeOverviewAnalytics(
      employees,
      period,
    );
  }
  ...
  Future<EmployeeDetailAnalytics> getDetailAnalytics({
    ...
    return _buildEmployeeDetailAnalytics(
      employee: employee,
      period: period,
    );
  }
...
@riverpod
EmployeeAnalyticsRemoteDataSource
    employeeAnalyticsRemoteDataSource(Ref ref) {
  ...
  return EmployeeAnalyticsRemoteDataSourceImpl(
...
/// Exception for employee analytics data fetch
/// failures.
class EmployeeAnalyticsDataException
  ...
  @override
  String toString() =>
      'EmployeeAnalyticsDataException: $message';
}

// ============================================================
// 6. MOCK DATA BUILDERS (used by Mock class)
// ============================================================

EmployeeOverviewAnalytics
    _buildEmployeeOverviewAnalytics(
  ...
  return EmployeeOverviewAnalytics(
    ...
    inactiveEmployees:
        math.max(0, inactiveEmployees),
    utilizationRate:
        _averageUtilization(employees, scale),
    ...
    roleDistribution:
        _buildRoleDistribution(employees),
    ...
    complianceItems:
        _buildOverviewCompliance(employees),
  );
}

EmployeeDetailAnalytics
    _buildEmployeeDetailAnalytics({
  ...
  final completedSessions = math.max(
    8,
    ((employee.reviewCount / 2) +
            _availableWeeklyHours(employee) * scale)
        .round(),
  );
  final contributionValue = _contributionValue(
    employee,
    completedSessions,
  );
  ...
  return EmployeeDetailAnalytics(
    ...
    utilizationRate:
        _employeeUtilization(employee, scale),
    ...
    scheduleLoad:
        _buildScheduleLoad(employee, scale),
    qualityMetrics:
        _buildQualityMetrics(employee),
    complianceItems:
        _buildDetailCompliance(employee),
  );
}
...
List<EmployeeTrendPoint> _buildEmployeeTrendPoints({
  ...
  return List<EmployeeTrendPoint>.generate(
    labels.length,
    (index) {
      final modifier =
          factors[(index + seed) % factors.length];
      return EmployeeTrendPoint(
        label: labels[index],
        sessions: math.max(
          4,
          sessionsBase * modifier * scale,
        ),
        contributionValue: math.max(
          500000,
          valueBase * modifier * scale,
        ),
      );
    },
  );
...
List<EmployeeRoleDistribution>
  ...
  return counts.entries
      .map(
        (entry) => EmployeeRoleDistribution(
          role: entry.key,
          count: entry.value,
        ),
      )
...
List<EmployeeComplianceItem>
    _buildOverviewCompliance(
  List<EmployeeEntity> employees,
) {
  final missingDocs = employees
      .where((employee) =>
          employee.verificationDocuments.isEmpty)
      .length;
  ...
  return [
    EmployeeComplianceItem(
      ...
      detail: missingDocs == 0
          ? 'All visible profiles have '
              'supporting documents.'
          : '$missingDocs profiles still need '
...
EmployeePerformanceSummary
  ...
  return EmployeePerformanceSummary(
    ...
    contributionValue: _contributionValue(
      ...
      math.max(
        8,
        (employee.reviewCount * scale).round(),
      ),
    ),
  );
}

List<EmployeeMixMetric> _buildEmployeeMix(
  EmployeeEntity employee,
) {
  switch (employee) {
    ...
    case SpaTherapistEntity spa:
      final items = spa.skills.isEmpty
          ? ['Facial', 'Device Care']
          : spa.skills;
      return _buildMixFromLabels(items);
    ...
    case BasicEmployeeEntity _:
      return _buildMixFromLabels(
        ['Support tasks', 'Client care'],
      );
  }
}

List<EmployeeMixMetric> _buildMixFromLabels(
  List<String> labels,
) {
  final total = labels.length;
  return List<EmployeeMixMetric>.generate(
    labels.length,
    (index) {
...
List<EmployeeScheduleLoad> _buildScheduleLoad(
  ...
  final available =
      _availableWeeklyHours(employee) /
      labels.length;

  return List<EmployeeScheduleLoad>.generate(
    labels.length,
    (index) {
      ...
      return EmployeeScheduleLoad(
        ...
        bookedHours: math.min(
          available,
          available * modifier * scale,
        ),
      );
    },
  );
...
List<EmployeeQualityMetric> _buildQualityMetrics(
  EmployeeEntity employee,
) {
  final docsReady =
      employee.verificationDocuments.isNotEmpty;
  final emergencyReady =
      employee.emergencyContactPhone != null;
  ...
  return [
    EmployeeQualityMetric(
      ...
      detail: '${employee.reviewCount} reviews '
          'across recent services',
    ...
    EmployeeQualityMetric(
      ...
      detail: docsReady
          ...
          : 'Upload credentials and identity '
              'records.',
      tone: docsReady
          ? AnalyticsStatusTone.positive
          : AnalyticsStatusTone.critical,
...
List<EmployeeComplianceItem>
    _buildDetailCompliance(
  EmployeeEntity employee,
) {
  return [
    EmployeeComplianceItem(
      title: 'Profile status',
      detail: '${_roleLabel(employee)} profile is '
    ...
    EmployeeComplianceItem(
      ...
      detail: employee
              .verificationDocuments.isEmpty
          ? 'No supporting verification '
              'documents attached.'
...
double _averageUtilization(
  ...
  final total = employees.fold<double>(
    0,
    (sum, employee) =>
        sum + _employeeUtilization(employee, scale),
  );
  return total / employees.length;
}

double _employeeUtilization(
  EmployeeEntity employee,
  double scale,
) {
  ...
  final bookedHours = math.min(
    ...
    availableHours *
        (0.58 +
            (employee.reviewCount % 18) / 100) *
        scale,
  );
  return (bookedHours / availableHours) * 100;
}

double _availableWeeklyHours(
  EmployeeEntity employee,
) {
  final workingSlots = employee.workSchedule
      .where((slot) => slot.isWorking);
...
double _contributionValue(
  EmployeeEntity employee,
  int completedSessions,
) {
  final sessionValue = switch (employee) {
    DoctorEntity doctor =>
      doctor.consultationFee ?? 500000.0,
    SpaTherapistEntity spa =>
      350000.0 + (spa.commissionRate * 10000),
