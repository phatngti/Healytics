...
class AdminFinanceWorkspaceNotifier
  ...
  void setPeriod(AdminFinancePeriod period) {
    state = state.copyWith(period: period, reloadToken: state.reloadToken + 1);
  }
  ...
  void setPartner(String? partnerId) {
    state = state.copyWith(
      filter: state.filter.copyWith(partnerId: partnerId),
      reloadToken: state.reloadToken + 1,
    );
  }
  ...
  void setCustomer(String? customerId) {
    state = state.copyWith(
      filter: state.filter.copyWith(customerId: customerId),
      reloadToken: state.reloadToken + 1,
    );
  }
  ...
  void setTransactionType(AdminFinanceTransactionType? value) {
    state = state.copyWith(
      filter: state.filter.copyWith(transactionType: value),
      reloadToken: state.reloadToken + 1,
    );
  }

  void setTransactionStatus(AdminFinanceTransactionStatus? value) {
    state = state.copyWith(
      filter: state.filter.copyWith(transactionStatus: value),
      reloadToken: state.reloadToken + 1,
    );
  }

  void setSettlementStatus(AdminFinanceSettlementStatus? value) {
    state = state.copyWith(
      filter: state.filter.copyWith(settlementStatus: value),
      reloadToken: state.reloadToken + 1,
    );
  }

  void setPayoutStatus(AdminFinancePayoutStatus? value) {
    state = state.copyWith(
      filter: state.filter.copyWith(payoutStatus: value),
      reloadToken: state.reloadToken + 1,
    );
  }

  void setRefundCaseStatus(AdminFinanceRefundCaseStatus? value) {
    state = state.copyWith(
      filter: state.filter.copyWith(refundCaseStatus: value),
      reloadToken: state.reloadToken + 1,
    );
  }

  void setReconciliationStatus(AdminFinanceReconciliationStatus? value) {
    state = state.copyWith(
      filter: state.filter.copyWith(reconciliationStatus: value),
      reloadToken: state.reloadToken + 1,
    );
  }
  ...
  void setAmountRange(double? min, double? max) {
    state = state.copyWith(
      filter: state.filter.copyWith(minAmount: min, maxAmount: max),
      reloadToken: state.reloadToken + 1,
    );
  }
  ...
  void setOnlyFlagged(bool value) {
    state = state.copyWith(
      filter: state.filter.copyWith(onlyFlagged: value),
      reloadToken: state.reloadToken + 1,
    );
  }
  ...
  void setOnlySlaBreached(bool value) {
    state = state.copyWith(
      filter: state.filter.copyWith(onlySlaBreached: value),
      reloadToken: state.reloadToken + 1,
    );
  }
...
final adminFinanceWorkspaceProvider =
    NotifierProvider<AdminFinanceWorkspaceNotifier, AdminFinanceWorkspaceState>(
...
final adminFinanceOverviewProvider = FutureProvider<AdminFinanceOverview>((
  ref,
) async {
...
final adminFinanceTrendProvider = FutureProvider<List<AdminFinanceTrendPoint>>((
  ref,
) async {
  final ws = ref.watch(adminFinanceWorkspaceProvider);
  return ref
...
final adminFinanceAlertsProvider = FutureProvider<List<AdminFinanceAlert>>((
  ref,
) async {
  final ws = ref.watch(adminFinanceWorkspaceProvider);
  return ref.read(adminFinanceRepositoryProvider).getAlerts(ws.period);
});

/// Partner exposure panel data.
final adminFinancePartnerExposureProvider =
    FutureProvider<List<AdminFinancePartnerExposure>>((ref) async {
      final ws = ref.watch(adminFinanceWorkspaceProvider);
      return ref
          .read(adminFinanceRepositoryProvider)
...
/// Finance export jobs.
final adminFinanceExportsProvider = FutureProvider<List<AdminFinanceExportJob>>(
  (ref) async {
    ref.watch(adminFinanceWorkspaceProvider.select((s) => s.reloadToken));
    return ref.read(adminFinanceRepositoryProvider).getExports();
  },
);
...
final adminFinanceTransactionDetailProvider =
    ...
    >((ref, id) async {
      ref.watch(adminFinanceWorkspaceProvider.select((s) => s.reloadToken));
      return ref.read(adminFinanceRepositoryProvider).getTransactionDetail(id);
    });

/// Single payout detail.
final adminFinancePayoutDetailProvider =
    FutureProvider.family<AdminFinancePayoutDetail, AdminFinancePayoutId>((
      ref,
      id,
    ) async {
      ref.watch(adminFinanceWorkspaceProvider.select((s) => s.reloadToken));
      return ref.read(adminFinanceRepositoryProvider).getPayoutDetail(id);
    });

/// Single refund case detail.
final adminFinanceRefundCaseDetailProvider =
    FutureProvider.family<
      AdminFinanceRefundCaseDetail,
      AdminFinanceRefundCaseId
    >((ref, id) async {
      ref.watch(adminFinanceWorkspaceProvider.select((s) => s.reloadToken));
      return ref.read(adminFinanceRepositoryProvider).getRefundCaseDetail(id);
    });
