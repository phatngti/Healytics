...
The feature was mock-backed; real mode threw `UnimplementedError`. Existing backend finance APIs are partner-scoped and do not satisfy the admin feature, so the integration requires a new admin finance contract first, then regenerated `admin_openapi` and Flutter datasource wiring.
