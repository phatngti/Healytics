//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:admin_openapi/api.dart';
import 'package:test/test.dart';

// tests for AdminLegalRepresentativeDto
void main() {
  // final instance = AdminLegalRepresentativeDto();

  group('test AdminLegalRepresentativeDto', () {
    // String fullName
    test('to test the property `fullName`', () async {
      // TODO
    });

    // String position
    test('to test the property `position`', () async {
      // TODO
    });

    // String idType
    test('to test the property `idType`', () async {
      // TODO
    });

    // String idNumber
    test('to test the property `idNumber`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // DateTime idIssueDate
    test('to test the property `idIssueDate`', () async {
      // TODO
    });

    // Object idFrontImgUrl
    test('to test the property `idFrontImgUrl`', () async {
      // TODO
    });

    // Object idBackImgUrl
    test('to test the property `idBackImgUrl`', () async {
      // TODO
    });

    // Object authorizationLetterUrl
    test('to test the property `authorizationLetterUrl`', () async {
      // TODO
    });

    // Object phoneNumber
    test('to test the property `phoneNumber`', () async {
      // TODO
    });


  });

}
