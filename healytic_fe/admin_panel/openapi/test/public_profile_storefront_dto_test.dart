//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:admin_openapi/api.dart';
import 'package:test/test.dart';

// tests for PublicProfileStorefrontDto
void main() {
  // final instance = PublicProfileStorefrontDto();

  group('test PublicProfileStorefrontDto', () {
    // Object coverImageUrl
    test('to test the property `coverImageUrl`', () async {
      // TODO
    });

    // Object logoImageUrl
    test('to test the property `logoImageUrl`', () async {
      // TODO
    });

    // Object description
    test('to test the property `description`', () async {
      // TODO
    });

    // List<String> gallery (default value: const [])
    test('to test the property `gallery`', () async {
      // TODO
    });

    // List<PublicProfileCertificationDto> certifications (default value: const [])
    test('to test the property `certifications`', () async {
      // TODO
    });


  });

}
