//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:admin_openapi/api.dart';
import 'package:test/test.dart';


/// tests for UserAppointmentsApi
void main() {
  // final instance = UserAppointmentsApi();

  group('tests for UserAppointmentsApi', () {
    // Get appointment details by ID
    //
    //Future<AppointmentResponseDto> userAppointmentControllerGetAppointment(String id) async
    test('test userAppointmentControllerGetAppointment', () async {
      // TODO
    });

    // Get service manual for an appointment
    //
    //Future<ServiceManualResponseDto> userAppointmentControllerGetServiceManual(String appointmentId) async
    test('test userAppointmentControllerGetServiceManual', () async {
      // TODO
    });

    // List all user appointments with optional distance calculation
    //
    //Future<List<AppointmentResponseDto>> userAppointmentControllerListAppointments({ num latitude, num longitude }) async
    test('test userAppointmentControllerListAppointments', () async {
      // TODO
    });

    // Get appointment categories for filter chips
    //
    //Future<List<AppointmentCategoryResponseDto>> userAppointmentControllerListCategories() async
    test('test userAppointmentControllerListCategories', () async {
      // TODO
    });

    // Get recent appointment activity for home dashboard
    //
    //Future userAppointmentControllerListRecentActivity({ num limit, num offset }) async
    test('test userAppointmentControllerListRecentActivity', () async {
      // TODO
    });

    // Get recommended services
    //
    //Future<List<RecommendedServiceResponseDto>> userAppointmentControllerListRecommendedServices() async
    test('test userAppointmentControllerListRecommendedServices', () async {
      // TODO
    });

  });
}
