...
class CartApi {
  ...
  ///
  /// * [ApplyCouponDto] applyCouponDto (required):
  Future<Response> cartControllerApplyCouponWithHttpInfo(String cartItemId, ApplyCouponDto applyCouponDto,) async {
    // ignore: prefer_const_declarations
    final path = r'/cart/{cartItemId}/coupon'
      .replaceAll('{cartItemId}', cartItemId);

    // ignore: prefer_final_locals
    Object? postBody = applyCouponDto;
  ...
  Future<CartItemResponseDto?> cartControllerApplyCoupon(String cartItemId, ApplyCouponDto applyCouponDto,) async {
    final response = await cartControllerApplyCouponWithHttpInfo(cartItemId, applyCouponDto,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// * [String] cartItemId (required):
  Future<Response> cartControllerRemoveCouponWithHttpInfo(String cartItemId,) async {
    // ignore: prefer_const_declarations
    final path = r'/cart/{cartItemId}/coupon'
      .replaceAll('{cartItemId}', cartItemId);
  ...
  Future<CartItemResponseDto?> cartControllerRemoveCoupon(String cartItemId,) async {
    final response = await cartControllerRemoveCouponWithHttpInfo(cartItemId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
