...
Healytics is a health and wellness platform. This is the `admin_panel` — a Flutter app in a monorepo workspace:

```
healytic_fe/
├── user_app/       # User application
├── admin_panel/    # This app
...
- Each remote data source file contains 3 parts:
  ...
  2. **Implementation class** (real API via the OpenAPI client)
...
Generated client lives under `./openapi`. Import generated DTOs only from the data layer:

```dart
import 'package:<openapi_package>/model/partner_request_dto.dart';
```

Regenerate when the backend OpenAPI spec changes.
