...
Connects a new API endpoint to the app following the data source pattern with OpenAPI client and proper error handling.

## Input Required
- `<feature_name>`: Feature this API serves
- `<endpoint>`: API endpoint path (e.g., `/api/v1/appointments`)
- `<method>`: HTTP method (GET, POST, PUT, DELETE)
- `<request_dto>`: Request DTO class (if any, from the OpenAPI client)
- `<response_dto>`: Response DTO class (from the OpenAPI client)
- `<entity_name>`: Domain entity to map to

## Steps

1. **Check OpenAPI client:** Verify the DTO exists in `./openapi`. If the DTO doesn't exist, the backend OpenAPI spec may need updating and the client regenerated.
