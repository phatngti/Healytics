...
///
/// Use [AppException.fromError] to convert any raw
/// error thrown by data-layer code into a typed
/// [AppException].
@freezed
sealed class AppException
  ...
  /// Network connectivity failure (socket, DNS,
  /// or timeout).
  ...
  ///
  /// Recognises [ApiException], [SocketException],
  /// [TimeoutException], and falls back to
  /// [UnexpectedException] for everything else.
  ...
  String get userMessage => switch (this) {
        ServerException(:final statusCode, :final message) =>
          _serverUserMessage(statusCode, message),
        NetworkException(:final message) => message,
        UnexpectedException(:final message) => message,
      };

  /// Maps common HTTP status codes to user-friendly
  /// messages; falls back to the raw server message.
  static String _serverUserMessage(
    int statusCode,
    String message,
  ) {
    return switch (statusCode) {
      ...
      409 => message.isNotEmpty
          ? message
          : 'A conflict occurred. '
              'Please try again.',
      422 => message.isNotEmpty
          ? message
          : 'Validation error. '
  ...
  /// Extracts the most meaningful message from an
  /// [ApiException], preferring the body content
