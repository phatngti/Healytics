...
class ChatSseService {
  ...
  const ChatSseService({required String basePath})
      : _basePath = basePath;
  ...
  /// which returns SSE events directly in the response
  /// body.
  ...
  /// Logs an error and forwards it to the
  /// [controller].
  ...
  void _emitEvent(
    ...
    if (type == null) {
      _log.warning(
        'Unknown SSE event type: $rawType',
      );
      return;
    }
    ...
    try {
      ...
      controller.add(
        ChatSseEvent(type: type, data: data),
      );
    } catch (e) {
      _log.severe(
