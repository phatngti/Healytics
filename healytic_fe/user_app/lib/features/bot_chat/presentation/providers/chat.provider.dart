...
class Chat extends _$Chat {
  ...
  ChatState build(String? conversationId) {
    ...
    // Cancel SSE subscription when provider is disposed.
    ref.onDispose(() {
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      final stream = _repository.sendMessageAndStream(convId, text);
      ...
      _sseSubscription = stream.listen(
        ...
        onDone: () {
          // Stream closed; finalize if still loading.
          if (state.isLoading) {
            _finaliseStream(
  ...
  void _appendToken(ChatSseEvent event, String segmentBaseId) {
    ...
    if (matches.isNotEmpty) {
      ...
      if (completed.isNotEmpty) {
        ...
        // Start a new pending message for remainder.
        _sentenceCounter++;
  ...
  /// 3. Resetting pending buffer for subsequent tokens
  void _handleRichEvent(
    ...
    // Reset pending for tokens after this rich event.
    _sentenceCounter++;
    _currentPendingId = '$segmentBaseId-s$_sentenceCounter';
    _pendingBuffer.clear();
  ...
  /// Returns a human-readable label for non-text types.
