...
CURL_OPTS="--cacert /certs/ca_certificate.pem -s -f -u ${RABBITMQ_DEFAULT_USER}:${RABBITMQ_DEFAULT_PASS}"

echo "=== RabbitMQ Init ==="
echo "Waiting for Management API at ${BASE_URL}..."

# Wait for Management API to be ready (up to 60s)
for i in $(seq 1 30); do
    ...
    if [ "$i" -eq 30 ]; then
        echo "ERROR: Management API not ready after 60s. Exiting."
        exit 1
    fi
    sleep 2
done
